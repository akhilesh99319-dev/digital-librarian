...
async function loadProfile() {
  // Pre-hydrate from cached session to prevent UI flash
  const cachedUser = api.getUser();
  if (cachedUser) {
    hydrateProfileUI(cachedUser);
  }
...
function hydrateProfileUI(u) {
  ...
  if (heading) heading.textContent = isMember ? 'Member Profile' : `${u.role || 'User'} Profile`;
  ...
  if (isMember && memberQRCard && memberQRContainer && qrEngine && u.id) {
    memberQRCard.style.display = 'block';
    const mCode = u.member_code || `MEM-${String(u.id).padStart(3, '0')}`;
    if (qrMemberName) qrMemberName.textContent = displayName;
